...
- **orientation.ts**: Central orientation abstraction. `needsPermissionRequest()` returns false for native. `requestOrientationPermission()` auto-grants in native and syncs permission to Zustand store. `startOrientationTracking()` provides unified event listener interface. `lockToLandscape()` uses Capacitor ScreenOrientation plugin on native, falls back to screen.orientation.lock on web. `unlockOrientation()` (async) uses Capacitor plugin on native, screen.orientation.unlock on web. `initOrientationSync()` and `syncPermissionFromStore()` keep module-level permission state in sync with Zustand store
- **statusbar.ts**: Status bar abstraction for native iOS. `hideStatusBar()` / `showStatusBar()` / `setStatusBarDark()` use Capacitor StatusBar plugin when native, no-op on web. Game.tsx hides status bar when entering landscape gameplay, restores on exit
